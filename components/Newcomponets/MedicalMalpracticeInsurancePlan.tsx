...
import MalpracticeCoverageCard from "@/components/Newcomponets/MalpracticeCoverageCard";
import Medicalspecialities from "@/components/Newcomponets/MedicalSpecialities";
import { planContents } from "@/lib/utilities";
const MedicalMalpracticeInsurance: React.FC = () => {
  const { t } = useTranslation("medicalmalpracticeinsuranceplan");
  return (
    <>
      <section className="bg-white">
        <div className="max-w-7xl mx-auto px-4 lg:px-8">
          <div className="max-w-7xl mx-auto px-4 lg:px-8 py-10 lg:mb-[15rem] rtl:mb-[20rem]">
            <Image
              src={MedicalmalpracticeImg}
              alt="Corporate Aviation"
            //   fill
               className="absolute inset-0 w-full h-full text-transparent mt-[4rem]"
              priority
            />
            <div className="relative z-10 p-10 md:p-20 max-w-lg text-black text-lg text-justify ml-0 lg:ml-[-11rem] mt-4 ">
              <h2 className="text-2xl md:text-2xl font-bold mb-4 text-[#10426c] ">
                {t("Malpracticeheader1")}
              </h2>
              <p className="mt-4 text-base md:text-lg text-align text-start ">
                {t("Malpracticeheader2")}
              </p>
            </div>
          </div>
      ...
      <MalpracticeCoverageCard/>
      <Medicalspecialities/>
      <div className="w-full max-w-7xl mx-auto mt-10 px-4">
        <Advantages content={planContents?.medicalmalpracticeinsuranceplan} />
      </div>
      <FAQs />
    </>
  );
